...
# Configura logging para aparecer no Render
logging.basicConfig(
    stream=sys.stdout,
    level=logging.INFO,
...
def perguntar_ao_modelo(pergunta: str) -> str:
    logger.info(f"perguntar_ao_modelo chamado. Chave presente: {bool(GROQ_API_KEY)}")
    if not GROQ_API_KEY:
...
def chat():
    ...
    logger.info(f"Recebido /chat: {dados}")
...
if __name__ == "__main__":
    if not GROQ_API_KEY:
        ...
        logger.info(f"GROQ_API_KEY presente. Modelo: {MODELO}")
    logger.info(f"ChromaBot iniciando na porta {PORTA}")
